"""
app.py -- Balfund Renko Trading System v3.1
LIGHT theme + auto lot size + multi-instrument + ATM/ITM/OTM + chart
"""
...
from engine import (INSTRUMENTS,DhanTokenManager,DhanAPI,api,resolve_instruments,
...
BG="#f0f2f5";CARD="#ffffff";ACC="#0369a1";GRN="#16a34a";RED="#dc2626";YEL="#ca8a04";TXT="#111827";DIM="#6b7280"
CARD_BD="#d1d5db";BTN_START="#16a34a";BTN_STOP="#dc2626";BTN_BLUE="#2563eb";BTN_GOLD="#ca8a04"

FT=("Segoe UI",20,"bold");FH=("Segoe UI",16,"bold");FM=("Segoe UI",14,"bold");FS=("Segoe UI",13);FX=("Consolas",12)
FL=("Segoe UI",12)  # label

class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Balfund Renko Trading System v3.1");self.geometry("1200x850");self.configure(fg_color=BG)
    ...
    def _build_ui(self):
        self.tv=ctk.CTkTabview(self,fg_color=CARD,segmented_button_fg_color="#e5e7eb",segmented_button_selected_color=ACC,
                                segmented_button_selected_hover_color="#0284c7",corner_radius=12)
    ...
    def _build_token(self):
        ...
        ctk.CTkButton(bf,text="Save Credentials",command=self._save_creds,fg_color=BTN_BLUE,hover_color="#1d4ed8",width=180,height=40,font=FM).pack(side="left",padx=6)
        ctk.CTkButton(bf,text="Generate Token",command=self._gen_tok,fg_color=BTN_START,hover_color="#15803d",width=180,height=40,font=FM).pack(side="left",padx=6)
        ctk.CTkButton(bf,text="Verify Token",command=self._verify_tok,fg_color=BTN_GOLD,hover_color="#a16207",width=160,height=40,font=FM).pack(side="left",padx=6)
    ...
    def _inp(self,p,label,show=""):
        ...
        e=ctk.CTkEntry(f,width=440,show=show or None,fg_color="#f9fafb",border_color=CARD_BD,text_color=TXT,font=FS);e.pack(side="left",padx=6)
        return e
    ...
    def _gen_tok(self):
        ...
        if not all([cid,pin,ts]): self.lb_tok.configure(text="Fill all fields first",text_color=RED);return
        def _g():
            ...
            if t:
                ...
                self.lb_tok.configure(text="Token generated and saved!",text_color=GRN)
            else: self.lb_tok.configure(text="Token generation failed",text_color=RED)
    ...
    def _verify_tok(self):
        ...
        self.lb_tok.configure(text="Token is VALID" if ok else "Token is INVALID",text_color=GRN if ok else RED)

    # ==================== INSTRUMENTS ====================
    def _build_control(self):
        f=self.t_ctrl
        ctk.CTkLabel(f,text="Instrument Control Panel",font=FT,text_color=ACC).pack(pady=10)
        self.scroll=ctk.CTkScrollableFrame(f,fg_color=BG,height=620)
    ...
    def _build_card(self,key):
        ...
        card=ctk.CTkFrame(self.scroll,fg_color=CARD,corner_radius=12,border_width=1,border_color=CARD_BD)
        card.pack(fill="x",padx=6,pady=5)

        # Row 1: Name + Status + Buttons
        r1=ctk.CTkFrame(card,fg_color="transparent");r1.pack(fill="x",padx=12,pady=(10,4))
        ctk.CTkLabel(r1,text=inst["label"],font=FH,text_color=TXT,width=240,anchor="w").pack(side="left")
        lbl_st=ctk.CTkLabel(r1,text="STOPPED",font=FM,text_color=DIM,width=250);lbl_st.pack(side="left",padx=10)
        btn_stop=ctk.CTkButton(r1,text="STOP",fg_color=BTN_STOP,hover_color="#b91c1c",width=90,height=36,font=FM,
                                command=lambda k=key:self._stop_inst(k),state="disabled")
        btn_stop.pack(side="right",padx=4)
        btn_start=ctk.CTkButton(r1,text="START",fg_color=BTN_START,hover_color="#15803d",width=90,height=36,font=FM,
        ...
        for label,val in fields:
            cf=ctk.CTkFrame(r2,fg_color="transparent");cf.pack(side="left",padx=8)
            ctk.CTkLabel(cf,text=label,text_color=DIM,font=FL).pack()
            e=ctk.CTkEntry(cf,width=70,fg_color="#f9fafb",border_color=CARD_BD,text_color=TXT,font=FS)
            e.insert(0,str(val));e.pack();ents[label]=e
        # LotSz label that shows auto (will be updated)
        lbl_lot=ctk.CTkLabel(r2,text="",font=("Segoe UI",10),text_color=DIM);lbl_lot.pack(side="left",padx=4)
        ents["lbl_lot"]=lbl_lot
        ...
        if inst["trade_type"]=="options":
            ...
            cmb_sm=ctk.CTkComboBox(r3,values=["ATM","ITM","OTM"],width=90,font=FS,fg_color="#f9fafb",border_color=CARD_BD,text_color=TXT)
            ...
            e_off=ctk.CTkEntry(r3,width=70,fg_color="#f9fafb",border_color=CARD_BD,text_color=TXT,font=FS)
        ...
        cmb_md=ctk.CTkComboBox(r3,values=["paper","live"],width=100,font=FS,fg_color="#f9fafb",border_color=CARD_BD,text_color=TXT)
    ...
    def _redraw_chart(self):
        try:
            ...
            for idx,key in enumerate(active):
                ...
                for s in ["bottom","left"]: ax.spines[s].set_color("#d1d5db")
                ax.grid(True,alpha=0.2,color="#9ca3af")
                gc=sum(1 for b in bricks if b.is_green);rc=len(bricks)-gc
                ax.set_title(f"{inst['label']} | {bricks[-1].close:.2f} @ {bricks[-1].time.strftime('%H:%M:%S')} | UP:{gc} DN:{rc} | Size={bs}",fontsize=12,fontweight="bold",color="#111827",pad=8)
                ...
                if tm and tm.current_trade and tm.current_trade.is_open:
                    ...
                    ax.set_xlabel(f"[{d}] {t.option_type}{int(t.strike) if t.strike else ''} @ {t.entry_price:.2f} | PnL={tm.total_pnl:+.2f}",fontsize=10,color="#0369a1",labelpad=6)
    ...
    def _build_dash(self):
        ...
        self.txt_log=ctk.CTkTextbox(f,height=300,fg_color="#f9fafb",text_color=TXT,font=FX,state="disabled",border_color=CARD_BD,border_width=1)
        ...
        try:
            ...
            for key in list(self.running_keys):
                ...
                if key not in self.dash_labels:
                    lf=ctk.CTkFrame(self.dash_frame,fg_color=CARD,corner_radius=8,border_width=1,border_color=CARD_BD);lf.pack(fill="x",padx=4,pady=3)
                ...
                if eng.bricks:
                    b=eng.bricks[-1];txt+=f" | Last={'UP' if b.is_green else 'DN'} {b.close:.2f}"
                t=tm.current_trade
                ...
                if "status" in w:
                    ...
                    elif tm.waiting_for_reversal: w["status"].configure(text="WAITING REVERSAL",text_color=YEL)
    ...
    def _ensure_ws(self):
        ...
        self.ws_thread=threading.Thread(target=self._ws_loop,daemon=True);self.ws_thread.start()
        return True
    ...
    def _init_inst(self,key):
        try:
            ...
            # Update lot size in GUI after auto-fetch
            w=self.inst_widgets.get(key,{}).get("ents",{})
            if "LotSz" in w and inst["lot_size"]>0:
                self.after(0,lambda:w["LotSz"].delete(0,"end"))
                self.after(0,lambda:w["LotSz"].insert(0,str(inst["lot_size"])))
                self.after(0,lambda:w.get("lbl_lot",ctk.CTkLabel(self)).configure(text="(auto)"))
            ...
            self.signal_secid_to_key[sig_sid]=key;self.trade_secid_to_key={}
            ...
            self.after(0,lambda:self._upd_ui(key,True))
            sm=inst.get("strike_mode","ATM")
            self.after(0,lambda:self._dlog(f"{inst['label']} STARTED | {inst['trade_mode'].upper()} | {sm} | Lot={inst['lot_size']}x{inst['lots']}"))
    ...
    def _stop_inst(self,key):
        ...
        self._upd_ui(key,False);self._dlog(f"{INSTRUMENTS[key]['label']} STOPPED")

    def _upd_ui(self,key,running):
        w=self.inst_widgets.get(key,{})
        if running:
